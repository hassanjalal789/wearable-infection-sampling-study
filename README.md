...
- **Records, not registration.** The protocol files labelled "preregistration" are preserved project records, listed in the [protocol index](docs/PROTOCOL_INDEX.md); several are still marked draft. They are not independently timestamped public preregistrations.
- **Verification status.** The values below were checked against the archived result files during preparation. On 25 September 2026, in a fresh environment, the published test suite passed (174 tests, or 181 with the two source workbooks the Gate B tests need); every published aggregate, including the BCa intervals, was recomputed from published files and matched; the result tables and digest were regenerated byte for byte and the figures matched visually; and the frozen power calculation's effect-ray cells and the Gate B reconstruction were re-run and matched their archived outputs. The full raw-data analysis has not been re-run and cannot be from this repository. The [verification report](docs/VERIFICATION_REPORT.md) and the [verification log](evidence/VERIFICATION_LOG.md) record each check with its date and outcome.

## Start here

1. [Research overview](docs/OVERVIEW.md) — the study, its result, its limits and its verification status on one page.
2. [Research summary](docs/RESEARCH_SUMMARY.md) — the study in plain language.
3. [Contribution statement](docs/CONTRIBUTIONS.md) — confirmed roles, advice received, and AI assistance.
4. [Methods guide](docs/METHODS_GUIDE.md) — the rules the executed study followed, each traced to the document that set it, with a [traceability review](docs/METHOD_TRACEABILITY.md) that checks each rule against the code.
5. [Data and cohort guide](docs/DATA_AND_COHORT.md) and [data availability](docs/DATA_AVAILABILITY.md) — the exact archives analysed, how each device and cohort decision was made, what each stage needs, and what is withheld.
6. [Energy and schedules guide](docs/ENERGY_AND_SCHEDULES.md) — how the modelled budget became seven ten-minute bursts, and where each schedule places them.
7. [Pipeline and tests guide](docs/PIPELINE_AND_TESTS.md) — the detector, calibration, runner and statistics, and what the tests show.
8. [Research timeline](docs/RESEARCH_TIMELINE.md) — when the design, execution, correction and sensitivity work happened, and which parts came before the result was known.
9. [Results and limitations](docs/RESULTS_AND_LIMITATIONS.md) — the main result and what constrains it.
10. [Evidence index](docs/EVIDENCE_INDEX.md) — which file supports which claim, and where the evidence stops.
11. [Reproduction gates and power](docs/REPRODUCTION_GATES_AND_POWER.md) — how far the upstream algorithms and published cohort numbers were reproduced, and what the frozen power calculation could detect.
12. [Historical execution log](evidence/historical_execution_log.md) — reviewed excerpts from the terminal record of the research, with a [guide](evidence/EXECUTION_LOG_GUIDE.md) to what they cover and what was left out.
13. [Reproducibility guide](docs/REPRODUCIBILITY.md) and [verification report](docs/VERIFICATION_REPORT.md) — what can be re-run from this repository, how, and what the checks for this publication showed.

The [reproducibility guide](docs/REPRODUCIBILITY.md) sets out what can be reproduced from this repository, level by level, with the commands I ran for this publication and what they reported. In short, after installing the pinned environment in [`environment/publication-requirements.txt`](environment/publication-requirements.txt), `python -m pytest tests/ -q` runs the test suite and `python src/check_public_package.py` rechecks the published links, arithmetic, intervals, energy audit and result tables. The seven Gate B tests also need two supplementary workbooks from the source studies, which are not redistributed; without them that module is not collected. The full raw-data analysis cannot be re-run from here; the guide explains why.
...
docs/
  ...
  METHOD_TRACEABILITY.md         each rule checked against the code, and where they differ
  ...
  OVERVIEW.md                    one-page research overview
  ...
  DATA_AVAILABILITY.md           source datasets, hashes, required inputs, exclusions
  REPRODUCIBILITY.md             what can be reproduced here, level by level, with commands
  VERIFICATION_REPORT.md         verification carried out for this publication
src/
  ...
  check_public_package.py        checks links, arithmetic, intervals, energy audit, tables (new code)
...
environment/                     historical environment records, a note on them, and
                                 the pinned verification environment (new)
results/
...
Every research artifact group has a published location or a stated reason for exclusion; the [manifest](evidence/PUBLICATION_MANIFEST.csv) lists each one.
...
- **Method review.** Checking the code against the protocol for this publication found planned rules and reporting items that the executed code did not carry out, among them the exclusion of daylight-saving transition days and the paired warning-time difference. Their effect cannot be measured from the published files. See the [method traceability review](docs/METHOD_TRACEABILITY.md).
