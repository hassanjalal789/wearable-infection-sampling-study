...
How each was used, and how far its results were reproduced, will be documented with the reproduction material, which is not yet published.
...
The data belong to the study teams. Neither the archives, the papers' supplementary files, nor any participant-level derivative is included here, and public availability is not treated as permission to redistribute them. Their use implies no affiliation with or endorsement by those teams. The [data and cohort guide](DATA_AND_COHORT.md) identifies the exact archives analysed.

## Software libraries

The published scripts use Python with NumPy and pandas. They are installed by the user, not bundled. Versions will be recorded with the environment records, which are not yet published.
