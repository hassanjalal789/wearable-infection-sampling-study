...
| The runner specification, the Phase-2 step-data correction, and the post-primary sensitivity operationalisation | A5, A6, A7 | **Not yet published.** Statements about them here were checked against the preserved records. |
...
| The authoritative outputs are the corrected run; an earlier run is superseded | [`METHODS_GUIDE.md`](METHODS_GUIDE.md) | The superseded run's outputs are not mixed into any table here, and neither run's files are published yet. |
