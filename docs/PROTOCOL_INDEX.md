...
| §8.5 energy ladder (E1–E4) as an analysis programme | Amendment A5 | Outcome analysis is limited to the central E3 budget; energy robustness is represented by the frozen feasibility audit |
| §7.1 rest condition (step count zero) applied to Phase 2 | Amendment A6 | Zero steps are inferred only for Phase-2 minutes with an observed heart rate and no explicit step record; this correction superseded the first run |
| §16 sensitivity analyses | Amendment A7 | Sensitivity analyses were operationalised after the primary result was seen, are sensitivity-only, and several planned analyses were not run, with reasons given |
...
4. A5, A6, A7 and the pre-outcome checkpoint — not yet published.
